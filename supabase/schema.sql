...
CREATE OR REPLACE FUNCTION "public"."get_user_latest_sets_for_exercises"("exercise_ids" "uuid"[]) RETURNS TABLE("id" "uuid", "exercise_id" "uuid", "reps" bigint, "weight" double precision, "time" bigint, "sort_order" bigint, "created_at" timestamp with time zone)
    LANGUAGE "sql" STABLE
    AS $$
WITH RankedSets AS (
    SELECT
...
ALTER FUNCTION "public"."get_user_latest_sets_for_exercises"("exercise_ids" "uuid"[]) OWNER TO "postgres";
...
GRANT ALL ON FUNCTION "public"."get_user_latest_sets_for_exercises"("exercise_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_latest_sets_for_exercises"("exercise_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_latest_sets_for_exercises"("exercise_ids" "uuid"[]) TO "service_role";
