...
interface WorkoutHistoryDao {
    ...
    suspend fun findWorkoutHistoriesByRange(
        start: Long,
        end: Long
    ): List<WorkoutHistoryWithWorkout>

    @Upsert
    suspend fun saveAll(histories: List<WorkoutHistory>): List<Long>

    @Transaction
    @Query("SELECT * FROM ${WorkoutHistory.TABLE_NAME} WHERE id IN (:ids) ORDER BY createdAt DESC")
    suspend fun findWorkoutHistoriesByIds(ids: List<String>): List<WorkoutHistoryWithWorkout>
}